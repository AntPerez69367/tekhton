...
setup_answers() {
    ...
    [[ -n "$overview_ans" ]] && save_answer "overview" "$overview_ans" || true
    [[ -n "$techstack_ans" ]] && save_answer "tech_stack" "$techstack_ans" || true
    [[ -n "$features_ans" ]] && save_answer "core_features" "$features_ans" || true
    [[ -n "$config_ans" ]] && save_answer "config_architecture" "$config_ans" || true
}
