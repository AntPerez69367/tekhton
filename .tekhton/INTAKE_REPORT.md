...
## Reasoning
- Scope is precise: new files, modified files, config keys, tests, and fallback table are all enumerated
- Status file schema is fully specified with an example JSON payload
- TUI layout is illustrated with ASCII art — no guessing about the visual design
- Acceptance criteria are specific and directly testable
- Migration Impact section is already present (PM-annotated from prior pass)
- One remaining gap: the Scope Summary lists "Shell files modified: 4" but the Integration Points section names 6 files — `lib/finalize.sh` (tui_complete call) and `lib/agent.sh` (tui_update_agent call) appear in Integration Points but are absent from the scope count. Corrected below.
