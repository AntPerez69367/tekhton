...
- [x] [2026-04-17 | "M94"] In `test_diagnose.sh` test 2b.6, `sed -i 's/## Notes/## Notes Hit max_turns in coder/'` inserts via GNU sed newline syntax — correct for WSL/Linux but would silently fail on BSD sed. Low risk given the platform, but worth noting for portability.
