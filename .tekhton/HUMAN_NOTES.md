...
## Bugs
- [ ] [BUG] Acceptance-criteria quality lint is emitted at end-of-run in lib/milestone_acceptance.sh (check_milestone_acceptance) so warnings are non-actionable. Move lint execution earlier to authoring paths: lib/draft_milestones_write.sh (during draft validation) and/or a pre-run validation hook before coder starts; keep milestone acceptance focused on pass/fail gates. Lint implementation is in lib/milestone_acceptance_lint.sh. <!-- note:n01 created:2026-04-21 priority:medium source:cli -->
