## Summary
M101 introduces `lib/output_format.sh` (new structured display formatters) and `lib/init_helpers_display.sh` (extracted detection-result rendering). The change is an internal ANSI output migration — no authentication, cryptography, network I/O, or user-supplied external input is involved. All data flows through pipeline-internal variables. Three low-severity hardening observations are noted; none are blocking.
