...
## Bugs
- [ ] [BUG] The current Architecture notes look like nonsense. The method for writing the observations must be broken because it's a mess. Remedy the problem and determine why it's no longer writing neat, orderly individual observations following the agreed upon format. The notes should be clear and actionable, not a jumbled mess.
- [ ] [BUG] The --auto-advance feature is not currently working. I recently ran it on Milestone 15.1 and it only completed one then asked me to commit. It should have continued on to the next milestone automatically. This is a critical feature for streamlining the workflow and reducing manual intervention, so it needs to be fixed.
