...
## Open
- [ ] [2026-04-05 | "[POLISH] The Watchtower text sizes are all fairly small, with some lines being nearly impossible to read like the "Milestone" column in the recent runs view. We should consider standard WCAG text sizes and spacing to improve readability."] Four decorative tag/chip classes (`.run-team-tag`, `.scout-sub-badge`, `.dep-chip-enabledby`, `.dep-chip-enables`) remain at 0.7rem = 10.5px at the new 15px base. This is still technically sub-WCAG for body text, but acceptable for small badge/tag elements where text is 1–4 characters and context makes meaning clear. Worth revisiting if contrast complaints arise.
