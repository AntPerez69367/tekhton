# Tekhton 5.0 — Rewrite Considerations: Maintainability, Safety, and Performance

> **Status: Pre-initiative stub.**
> This document captures architectural observations from the V3/V4 era for
> consideration when scoping V5. No milestones or implementation decisions have
> been made. Review after V4 is feature-complete before committing to anything
> here.
...
4. **Parallelism** — V4's parallel execution engine was difficult to implement
...
V4 added parallel milestone execution. In shell, this means `&` background
...
**Tentative recommendation (for V5 scoping):** Python, as an incremental
...
Before committing V5 resources to a rewrite, answer these questions:

- Is V4 complete and stable enough to be the baseline for a rewrite?
...
If the answers are mostly yes, Phase 1 (data layer) is a safe V5 M01 candidate.
If uncertain, defer to V6 and instead invest V5 in consolidating the shell
codebase (reducing file count, strengthening types via `declare`, improving test
ergonomics with better bats patterns).

---

*Last updated: April 2026. Revisit after V4 feature-complete milestone.*
